...
func mockAnthropic(t *testing.T, ttftDelay time.Duration) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		f.Flush()
		fmt.Fprint(w, "event: content_block_delta\ndata: {\"delta\":{\"type\":\"text_delta\",\"text\":\" friend\"}}\n\n")
		f.Flush()
...
func mockBedrock(t *testing.T, ttftDelay time.Duration) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		time.Sleep(ttftDelay)
		writeAWSEvent(w, "contentBlockDelta", []byte(`{"delta":{"text":"Hello"}}`))
		writeAWSEvent(w, "contentBlockDelta", []byte(`{"delta":{"text":" there"}}`))
		writeAWSEvent(w, "contentBlockDelta", []byte(`{"delta":{"text":" friend"}}`))
		writeAWSEvent(w, "metadata", []byte(`{"usage":{"outputTokens":3}}`))
	}))
}
